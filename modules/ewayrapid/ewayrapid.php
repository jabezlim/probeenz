...
class Ewayrapid extends PaymentModule
{
	...
	public function hookPayment($params)
	{
		...
		if ($currency->iso_code != 'NZD') {
			...
			//echo 'card>>>'.$card_amount.','.$currency->conversion_rate.','.$total_amount;
		} else {
	...
	public function getAccessCodeResult()
	{
		...
		if ($currency->iso_code != 'NZD') {
			$order_total = number_format($cart->getOrderTotal(), 2, '.', ''); //(float)($result->TotalAmount * $currency->conversion_rate/100);
		} else {
